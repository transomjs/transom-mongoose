...
module.exports = function ModelUtils() {
	...
	function mapToSchemaType(attribute) {
		const DEFAULTS = {
			minStringLength: 0,
		...
		let result;
		const schemaType = (attribute.type || 'string').toLowerCase();
		switch (schemaType) {
			case "objectid":
			case "connector":
				result = {
			...
			case "string":
				result = {
					type: 'string',
					minlength: attribute.min || DEFAULTS.minStringLength,
					maxlength: attribute.max || DEFAULTS.maxStringLength
				};
				break;
			case "number":
				result = {
				...
				if (attribute.min !== undefined) {
					result.min = attribute.min;
				}
				if (attribute.max !== undefined) {
					result.max = attribute.max;
				}
				break;
			case "binary":
		...
		return function (opts) {
			for (key in this.schema.paths) {
				if (this[key] && typeof this[key] === "string") {
					...
					const dataType = (this.schema.paths[key].options.type || 'string').toLowerCase();
