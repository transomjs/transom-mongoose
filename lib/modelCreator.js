...
module.exports = function ModelCreator(options) {
	...
	function createSchema(options, entity) {
		...
		for (const attrib of attributes) {
			...
			if (includeAttribute) { //_id will be added at the end.
				...
				if (schemaType === 'virtual') {
					...
					schemaObject[attrib.code] = modelUtils.mapToSchemaType(schemaType, CUSTOM_TYPEKEY);
		...
		if (entity.audit !== false) {
			timestamps = {
				createdAt: (entity.audit ? entity.audit.createdAt : null) || 'created_date',
				updatedAt: (entity.audit ? entity.audit.updatedAt : null) || 'updated_date'
			};
		}
	...
	function createEntities(dbMongoose) {
		const seedPromises = [];
		...
		Object.keys(dbMongoose).forEach(function (key) {
			...
			entity.collection = dbMongoose[key].collection;
