...
module.exports = function ModelCreator(options) {
	...
	const customTypeKey = 'type'; // Mongoose v6 doesn't support this anymore!
	...
	function insertSeedData(model, seed) {
		...
		seed.map(function (record) {
			record._id = record._id || new Types.ObjectId().toHexString();
	...
	function createEntities(options) {		
		...
		return Promise.all(seedPromises).then(() => {
