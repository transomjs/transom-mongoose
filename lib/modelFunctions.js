...
module.exports = function ModelFunctions(options) {
    ...
    function modelFind(model, req) {
        return new Promise(function (resolve, reject) {
            ...
            if (typeof query.aclRead === 'function') {
                debug('Adding ACL Read to find() query');
                query.aclRead(req);
            }
            ...
            query.exec()
                .then(function (items) {
                    ...
                    debug('Error executing model find()', err);
                    reject(err);
                });
        });
    }
    ...
    function modelFindById(model, req) {
        return new Promise(function (resolve, reject) {
            ...
            try {
                ...
                debug('Invalid Id in findById:', req.params.__id);
            ...
            if (operations['_connect']) {
                debug('Adding Connect operation to findById query');
            ...
            // Make sure the model uses ACL before calling it.
            if (typeof query.aclRead === 'function') {
                debug('Adding ACL Read to findById query');
                query.aclRead(req);
            }
            ...
            query.exec()
                .then(function (item) {
                    if (!item) {
                        debug('Model findById() record not found');
                    ...
                    debug('Error executing model findById()', err);
                    reject(err);
                });
        });
    }
    ...
    function modelFindBinary(model, req) {
        return new Promise(function (resolve, reject) {
            const isBinary = model.schema.paths[req.params.__attribute].options.type.isBinary;
            ...
            try {
                ...
                debug('Invalid Id in findBinary:', req.params.__id);
                return reject(new restifyErrors.InvalidArgumentError('Invalid ID format'));
            }
            ...
            // Make sure the model uses ACL before calling it.
            if (typeof query.aclRead === 'function') {
                debug('Adding ACL Read to findBinary query');
                query.aclRead(req);
            }
            ...
            query.exec()
                .then(function (item) {
                    if (!item || !item[req.params.__attribute]) {
                        debug('Item or Attribute not found in findBinary query');
                        return reject(new Error('Not Found'));
                    }
                    resolve(item[req.params.__attribute]);
                }).catch(function (err) {
                    debug('Error executing model findBinary()', err);
                    reject(err);
                });
        });
    }
    ...
    function modelCount(model, req) {
        return new Promise(function (resolve, reject) {
            ...
            // Make sure the model uses ACL before calling it.
            if (typeof query.aclRead === 'function') {
                debug('Adding ACL Read to modelCount query');
                query.aclRead(req);
            }
            ...
            query.exec().then(function (count) {
                ...
                debug('Error executing modelCount query');
                reject(err);
            });
        });
    }
                ...
                .then(function (doc) {
                    ...
                    for (var key in options) {
                        if (!(key === '_id' || key === model.schema.options.versionKey || key === model.schema.options.aclKey)) {
                            ...
                            if (model.schema.paths[key].options.type.isBinary) {
