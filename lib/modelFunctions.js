...
module.exports = function ModelFunctions(options) {
    ...
    function modelFindById(entity, req) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            const selectOpts = handlerUtils.processSelectOperator(model, operations['_select'], customTypeKey);
    ...
    function modelFindBinary(entity, req) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            const isBinary = pathOpts[customTypeKey] && pathOpts[customTypeKey].__type === 'binary';
            ...
            for (var key in model.schema.paths) {
                if (!(key === model.schema.options.versionKey || key === model.schema.options.aclKey)) {
                    if (model.schema.path(key) && reqContent[key]) {
                        ...
                        if (pathOpts[customTypeKey] && pathOpts[customTypeKey].__type === 'binary') {
            ...
            query.exec().then(function (doc) {
                ...
                for (var key in options) {
                    if (!(key === '_id' || key === model.schema.options.versionKey || key === model.schema.options.aclKey)) {
                        ...
                        const pathOpts = model.schema.path(key).options;
                        if (pathOpts[customTypeKey] && pathOpts[customTypeKey].__type === 'point') {
                        ...
                        } else if (pathOpts[customTypeKey] && pathOpts[customTypeKey].__type === 'binary') {
                            if (typeof newValue === 'object' && newValue.path) {
                                doc[key] = {
                                    binaryData: fs.readFileSync(newValue.path),
                                    filename: newValue.name,
