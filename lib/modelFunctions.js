...
module.exports = function ModelFunctions(options) {
    ...
    function modelFindById(entity, req) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            const selectOpts = handlerUtils.processSelectOperator(model, operations['_select'], CUSTOM_TYPEKEY);
    ...
    function modelFindBinary(entity, req) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            const isBinary = pathOpts[CUSTOM_TYPEKEY] && pathOpts[CUSTOM_TYPEKEY].__type === 'binary';
    ...
    function modelInsert(entity, req) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            var reqContent = Object.assign({}, req.body, req.files);

            for (var key in model.schema.paths) {
                if (!(key === model.schema.options.versionKey || key === model.schema.options.aclKey)) {
                    if (model.schema.path(key) && reqContent[key]) {
                        // Apply the provided value to the ModelInstance
                        const pathOpts = model.schema.path(key).options;
                        if (pathOpts[CUSTOM_TYPEKEY] && pathOpts[CUSTOM_TYPEKEY].__type === 'binary') {
                            var newValue = reqContent[key];
                            ...
                            modelInstance[key] = reqContent[key];
                        }
                    } else {
            ...
            query.exec().then(function (doc) {
                ...
                for (var key in options) {
                    if (!(key === '_id' || key === model.schema.options.versionKey || key === model.schema.options.aclKey)) {
                        ...
                        const pathOpts = model.schema.path(key).options;
                        if (pathOpts[CUSTOM_TYPEKEY] && pathOpts[CUSTOM_TYPEKEY].__type === 'point') {
                        ...
                        } else if (pathOpts[CUSTOM_TYPEKEY] && pathOpts[CUSTOM_TYPEKEY].__type === 'binary') {
                            if (typeof newValue === 'object' && newValue.path) {
                                doc[key] = {
                                    binaryData: fs.readFileSync(newValue.path),
                                    filename: newValue.name,
