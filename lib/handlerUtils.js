...
module.exports = function HandlerUtils() {
	...
	function processSelectOperator(model, select) {
		...
		if (select) {
			...
			for (var path in paths) {
				...
				if (pathOpts.type && pathOpts.type.isBinary) {
					// Don't include the binary data unless asked for specifically.
					selectObj.applyRoot = true;
	 ...
	 * @param {Model} model - The mongoose model that represents the entity we are querying.
	 * @return query
	 */
	function buildQuery(query, req, model) {
		...
		for (var key in currentAttribs) {
			...
			for (var val in value) {
				...
				args.push(getDataTypeClause(model, key, value[val], req));
			}
		}
		// } catch (err) {
	...
	function applyConnectOperator(options) {
		...
		if (connect.populateReverse.length > 0) {
			...
			for (var i = 0; i < connect.populateReverse.length; i++) {
				...
				var model = mongoose.model(pr.entity);
				if (!model) {
	...
	//attempt to return the strongly typed value based on the string value in the first argument
	function getStrongTypeValue(val, datatype) {

		var retval = val; // default as-is.
		try {
			// If the datatype comes in as an ObjectId (or some other), get it's name as a string.
			if (datatype.name) {
			...
			switch (datatype) {
				case "ObjectId":
					try {
						retval = new mongoose.Types.ObjectId(val);
					} catch (err) {
	 ...
	 */
	function getDataTypeClause(model, key, value, req) {
		var arg = {};

		var datatype = model.schema.paths[key].options.type;
		if ('~' === value[0]) {
			if (datatype !== "string") {
		...
		if (arg[key] === "CURRENT_USERNAME" && req.locals && req.locals.user && req.locals.user.username) {
			arg[key] = req.locals.user.username;
		}
		return arg;
	}
