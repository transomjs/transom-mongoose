	...
	function applyValues(doc, values, model, isInsert) {
		...
		for (let key in values) {
			...
			if (key === '_id') {
				...
				// Noop
			} else if (newValue && model.schema.path(key)) {
				const pathOpts = model.schema.path(key).options;
				if (pathOpts[typeKey] && pathOpts[typeKey].__type === 'point') {
				...
				} else if (pathOpts[typeKey] && pathOpts[typeKey].__type === 'binary') {
				...
				} else if (pathOpts[typeKey] && pathOpts[typeKey].constructor.name === 'Array') {
					const arrayValue = tryParseJSON(newValue, key);
					doc[key] = (typeof arrayValue === 'string' ? [arrayValue] : arrayValue);

				} else if (pathOpts[typeKey] && pathOpts[typeKey] === 'mixed') {
