...
module.exports = function HandlerUtils(options) {
		...
		for (let connector of connectors) {
			if (connector.indexOf('.') === -1) {
				...
				if (!model.schema.paths.hasOwnProperty(connector)) {
					throw createError(400, 'Invalid attribute code in _connect: ' + connector);
				}
				...
				if (select[connector]) {
					...
					if (sel.length > 0) {
						...
						if (!tmpModel) {
							throw createError(400, 'Attribute cannot be used with _connect: ' + connector);
						}
					}
					tmp.select = sel.join(',');
				}
				...
				if (!mongoose.model(modelPrefix + parts[0])) {
					throw createError(400, 'Invalid entity code in _connect: ' + connector);
				}
	...
	function buildQuery(query, req, entity) {
		...
		// Add a named collation to the query.
		if (currentOps._collation) {
			const namedCollation = collations[currentOps._collation];
			if (namedCollation) {
				debug(`Query is using named collation: ${currentOps._collation}`);
				query.collation(namedCollation);	
			} else {
				throw createError(400, `Named collation does not exist: ${currentOps._collation}`);
			}
		}
	...
	function applyConnectOperator(options) {
		...
		if (connect.populateReverse.length > 0) {
			...
			for (var i = 0; i < connect.populateReverse.length; i++) {
				...
				if (!model) {
					throw createError(400, `Invalid Entity used in _connect: ${pr.entity}.${pr.attribute}`);
				}
				if (!model.schema.paths.hasOwnProperty(pr.attribute)) {
					throw createError(400, `Invalid Attribute used in _connect: ${pr.entity}.${pr.attribute}`);
				}
	...
	function getDataTypeClause(model, key, value, user) {
		...
		const rawDatatype = model.schema.paths[key].options.__type; // As found in the Transom API Entity definition
		if ('~' === value[0]) {
			if (datatype === model.base.SchemaTypes.String || datatype === 'string' || rawDatatype === 'string') {
				if ('>' === value[1]) {
					// Begins with
		...
		} else if (String(value).toLowerCase() === "!isnull") { //H+ for !=
			// Not Null
			arg[key] = {
				$ne: null
			};
